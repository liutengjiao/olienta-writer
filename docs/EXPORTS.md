...
- Word：生成 `.docx` 文件，包含封面、目录、章节/字数/段落统计、中文字体默认值、章节分页、页眉和页脚页码/总页数；正文段落带首行缩进。
...
正文页保留“导出本章”的轻入口；完整导出控制从“继续写”页明显到达，不再挂在项目结构里的“工具与设置”组下。导出页提供全书、当前章、选中章节三组操作，每组都支持 Markdown、TXT 和 Word。
...
- Word 导出是当前版本 OpenXML，已能生成 `exports/manuscript.docx`。
- 当前支持标题、普通段落、引用、分隔线、无序列表、编号列表和代码块；正文段落带首行缩进。
- Word 文件包含封面、目录、章节数、字数统计、段落统计、默认中文字体、段落行距、一级标题居中、后续一级标题分页、页眉和页脚页码/总页数。
- 目录页包含更新提示：打开 Word 后可按 Ctrl+A，再按 F9 更新目录页码和页数字段。
- 
- 导出不会修改原始章节文件。
